if (window.jQuery || window.Zepto) {
	(function ($) {
		var TransferTree3 = function (selector, config) {
			/*
			高亮样式设置
			*/
			 ...
			 */
			var defaults = {
				model:'only',   //only 进选择人员 ellipsis 省略全选的人员使用部门代替
				zNodes: [],
				setting: {
					check: {
						enable: true
		...
		transfertree3.prototype = {
            ...
            changeSelect:function (){
			    debugger
                $('.msg_alert').find('.tree-change>a').unbind('click').on('click',function(){
                    debugger
			...
			clickFn: function (event, treeId, treeId) {
				...
				var zTree = $.fn.zTree.getZTreeObj("treeDemo_doulbe");
			...
			*/
			checkFn: function (event, treeId, treeNode) {
				...
				$(".choose-content-doulbe").html(rhtml);
			...
			removeData: function () {
				var zTree = this.getzTree();
				//右侧选框删除事件
				$(".choose-content-doulbe").on("click", ".close", function () {
			...
			searchData: function () {
				...
				*/
				function updateNodes(highlight, nodeList) {
					if ($("#filter-text-doulbe").val() != "") { zTree.expandAll(false); }//先折叠所有节点，再展开相应的节点
				...
				function searchNode() {
					var filter = $("#filter-text-doulbe").val();
				...
				/*输入框添加输入监听*/
				$("#filter-text-doulbe").on("propertychange", searchNode).on("input", searchNode);
			}
		}
		$.fn.TransferTree3 = function (params) {
			var that = $(this);
			var instance = new transfertree3(that[0], params);
			instance.init();
			return instance;
		}
