if (window.jQuery || window.Zepto) {
	(function ($) {
		...
		transfertree.prototype = {
			...
			*/
			checkFn: function (event, treeId, treeNode,m) {
				var _this = this;
				...
				$.each(checkedNode, function (i, item) {
					if(item.isParent==true){
						if(item.check_Child_State==2){
							parIdArr.push(item.id);
							if(parIdArr.indexOf(item.pId)==-1){
								parArr.push(item);
							}
						}
					}
					if(!item.isParent && parIdArr.indexOf(item.pId)==-1){
						chilArr.push(item);
					}
				...
				var rightArr=parArr.concat(chilArr)
				var rhtml = "";
				$.each(rightArr, function (i, item) {
					if(item.isParent){
						//插入父级
						rhtml += "<li data-item='" + JSON.stringify(item) + "'><span>组" + item.name + "</span><span class='close'>x</span></li>";
					} else {
						//插入子级
